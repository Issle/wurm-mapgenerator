 ...
 */
@AppModule("simplex")
public class SimplexNoiseGenerator extends NoiseGenerator { // Simplex noise in
	
	private float persistance = 0.5f;
	private int octaveCount=8;
	...
	public float[][] generateImpl(int size) {
		...
		float localPersistance = 1f;
		for(int oc = octaveCount; oc > 0; oc--)
		{
			double denom = Math.pow(2, oc);
			for (int i = 0; i < size; i++)
				for (int j = 0; j < size; j++)
					output[i][j] += localPersistance*(float) noise(i/denom, j/denom);
